# return item
summon minecraft:item ~ ~ ~ {Item:{id:"minecraft:iron_block",Count:1b,tag:{isGasCompressor:1b,display:{Name:"{\"text\":\"§f气体压缩机\"}"},CustomModelData:6978073}}}
...
kill @s
execute positioned ~ ~-1.06 ~ run kill @e[tag=smdcompressor_texture,sort=nearest,distance=..0.01,limit=1]
execute positioned ~-0.45 ~0.1 ~ run kill @e[tag=smdcompressor_gas,sort=nearest,distance=..0.01,limit=1]
execute positioned ~ ~-0.5 ~-0.06 run kill @e[tag=smdcompressor_cylinder,sort=nearest,distance=..0.01,limit=1]