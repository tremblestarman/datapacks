...
summon minecraft:area_effect_cloud ^ ^ ^1 {Tags:[ori_point]}
...
scoreboard players remove y2 var 1200

execute positioned ~ ~0.0125 ~ run tag @e[type=pig,tag=3mg_fly,sort=nearest,limit=1] add force_add
execute as @e[tag=force_add] store result entity @s Motion[0] double 0.00002 run scoreboard players get x2 var
execute as @e[tag=force_add] store result entity @s Motion[1] double 0.00002 run scoreboard players get y2 var
execute as @e[tag=force_add] store result entity @s Motion[2] double 0.00002 run scoreboard players get z2 var
